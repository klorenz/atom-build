...
module.exports = {
  ...
  selectActiveTarget: function() {
    var _ = require('lodash');
    var targetsView = require('./targets-view').create();
    ...
    }).catch(BuildError, function (error) {
  ...
  startNewBuild: function(source) {
    ...
    Promise.resolve()
      ...
      .then(function (targets) {
        ...
        atom.notifications.addError(error.name, { detail: error.message });
      }).catch(SyntaxError, function (error) {
        atom.notifications.addError('Invalid build file.', { detail: 'You have a syntax error in your build file: ' + error.message });
      }).catch(function (error) {
        atom.notifications.addError('Ooops. Something went wrong.', { detail: error.message + (error.stack ? '\n' + error.stack : '') });
      });
  },
  ...
  build: function(source) {
    ...
    if (!this.errorMatcher) {
      this.errorMatcher = ErrorMatcher.create();
    ...
    this.doSaveConfirm(this.unsavedTextEditors(), function() {
      var next = this.startNewBuild.bind(this, source);
  ...
  doSaveConfirm: function(modifiedTextEditors, continuecb, cancelcb) {
    ...
    this.saveConfirmView = require('./save-confirm-view').create();
